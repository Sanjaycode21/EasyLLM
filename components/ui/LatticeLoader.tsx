...
const PATTERNS: Record<string, Record<number, any>> = {
...
const MARKS: Record<number, Record<string, number[]>> = {
  3: { done: [2, 3, 5, 7], error: [0, 2, 4, 6, 8] },
  4: { done: [7, 8, 10, 13], error: [0, 3, 5, 6, 9, 10, 12, 15] }
};

const resolvePattern = (pattern: any, grid: number) => {
  ...
  const max = Math.max(0, ...cells.filter((v: any) => v != null));
    ...
    >
      <span className="lattice-loader__grid" aria-hidden="true">
        <span className="lattice-loader__layer lattice-loader__run">
          {pat.cells.map((unit: any, i: number) => (
            <span
              key={i}
